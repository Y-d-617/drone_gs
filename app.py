    ...
    with col_gauges:
        ...
        if st.session_state.mission_active and not st.session_state.mission_paused:
            if random.random() < 0.2:
                telemetry_data = {
                    "位置": {"lat": pos[1], "lng": pos[0], "alt": 50},
                    "速度": st.session_state.flight_speed,
                    "电池": st.session_state.battery
                }
                st.session_state.comm_link.send_telemetry("FCU", "GCS", "实时状态", telemetry_data)
